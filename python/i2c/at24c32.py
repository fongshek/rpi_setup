...
class at24c32:
    ...
    def random_write_byte(self, add, data):
        vals = [add&0xff,data]
        self.i2c.bus.write_i2c_block_data(DEVICE_ADDRESS,add>>8,vals)  
        time.sleep(self.WRITE_CYCLE_TIME)
        
    def random_write_bytes(self, add, bytes):
        if (len(bytes) <= 31) :
            vals = [add>>8] + [add&0xff] + bytes
            self.i2c.bus.write_i2c_block_data(DEVICE_ADDRESS,vals[0],vals[1:])  
            time.sleep(self.WRITE_CYCLE_TIME)
        else :
            vals = [add>>8] + [add&0xff] + bytes
            self.i2c.bus.write_i2c_block_data(DEVICE_ADDRESS,vals[0],vals[1:33])  
            time.sleep(self.WRITE_CYCLE_TIME)
...
if (1):
    ...
    my24c32 = at24c32(DEVICE_ADDRESS,0)
