...
def on_update_1602IO(client,data,cs,rs,rw,bl):
    print "update 1602IO from " + client.name , format(data,'02x'), cs, rs, rw, bl


def main():
    myDevice = lcd1602("myLCD",0)
    myDevice.on_update_io = on_update_1602IO
    myDevice.init_4bitmode();
    myDevice.test();    
